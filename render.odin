...
vt_set_color :: proc(mode: i32, col: Pixel) {
	f.printf(CSI + "%d;2;%d;%d;%dm", mode, col.b, col.g, col.r)
}
...
put_pixel_cb :: #type proc(x, y: i32, p: Pixel)
		...
		for i in 0 ..< WIDTH {
			...
			for m in 0 ..= 1 {
				...
				for mn in 0 ..< MULTISAMPLE_NUMBER {
					...
					nudge *= APERTURE
					sray := Ray {
						ORIGIN + Point30(nudge),
						bl_vec + xp * hc_vec + yp * vc_vec - nudge,
					}
			...
			vt_set_color(48, pp[0])
			vt_set_color(38, pp[1])
			f.print(block)
...
compute_ray_color :: proc(rayp: ^Point30, rayv: ^Vec30, stack_depth: int) -> (color: Color3) {
	...
	for sphere, tsi in sphere_array {
		...
		temp_t, temp_intersected := ray_sphere_intersection(
			rayv,
			rayp,
			&sphere_array[tsi].c,
			sphere.r,
